...
{
    public partial class mst_City : System.Web.UI.Page
    {
        private string form_PmryCol = "MiscID";
        private string form_tbl = "tbl_MiscellaneousMaster";
        private string Grid_fn = "fn_MiscCityView()";
        static int iModuleID = 0;

        protected void Page_Load(object sender, EventArgs e)
        {
            CommonLogic.SetMySiteName(this, "Admin :: City Master", true, true, true);
            if (!Page.IsPostBack)
            {
                iModuleID = Localization.ParseNativeInt(DataConn.GetfldValue("SELECT ModuleID FROM tbl_ModuleSettings Where PageLink='mst_City.aspx'"));
            }
            ...
            ddl_State.Attributes.Add("onFocus", "nextfield ='" + ddl_District.ClientID + "';");
            ddl_District.Attributes.Add("onFocus", "nextfield ='" + txtCity.ClientID + "';");
            txtCity.Attributes.Add("onFocus", "nextfield ='" + btnSubmit.ClientID + "';");
            #endregion
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                strNotIn = " MiscID <>" + iPmryID + " and GroupID= 4";
            }
            catch
            {
                iPmryID = 0;
            ...
            //strNotIn = ((strNotIn.Length == 0) ? "" : (strNotIn + " And ")) + " GroupID = 4 ";

            if (Localization.ParseNativeInt(DataConn.GetfldValue("Select COUNT(0) from  " + form_tbl + " WHERE MiscName = " + CommonLogic.SQuote(txtCity.Text.Trim()) + " and ParentID=" + ddl_District.SelectedValue + (strNotIn.Length > 0 ? " and " + strNotIn : ""))) > 0)
            //if (!commoncls.IsUniqueEntry(form_tbl, "MiscName", txtCity.Text.Trim(), strNotIn))
            {
                AlertBox("Duplicate City Name Not Allowed !", "", "");
            }
            else
            {
                string strQry;
                if (iPmryID == 0)
                {
                    strQry = string.Format("insert into tbl_MiscellaneousMaster values({0}, {1}, 4)", CommonLogic.SQuote(txtCity.Text.ToUpper().Trim()), ddl_District.SelectedValue);
                }
                else
                {
                    strQry = string.Format("Update tbl_MiscellaneousMaster Set MiscName = {0}, ParentID = {1} where MiscID = {2}", CommonLogic.SQuote(txtCity.Text.ToUpper().Trim()), ddl_District.SelectedValue, iPmryID);
                }
                if (DataConn.ExecuteSQL((strQry + ";--").Replace("''", "Null"), iModuleID, 0) == 0)
                {
                    ...
                    if (iPmryID == 0)
                    {
                        AlertBox("City Added successfully...", "", "");
                    }
                    else
                    {
                        AlertBox("City Updated successfully...", "", "");
                    }
                    ClearContent();
                }
                else
                {
                    AlertBox("Error occurs while Adding/Updateing City Name, please try after some time...", "", "");
                }
            }
        }
        ...
        {
            txtCity.Text = "";
            ccd_Country.SelectedValue = "0";
            ccd_State.SelectedValue = "0";
            ccd_District.SelectedValue = "0";
                ...
                switch (e.CommandName)
                {
                    case "RowUpd":
                        ...
                        using (IDataReader iDr = DataConn.GetRS(string.Format("Select * from {0} where {1} = {2} ", Grid_fn, form_PmryCol, e.CommandArgument)))
                        {
                            if (iDr.Read())
                            {
                                txtCity.Text = iDr["MiscName"].ToString();
                    ...
                    case "RowDel":
                        if (commoncls.IsDeleted(commoncls.ComboType.City, Localization.ParseNativeInt(e.CommandArgument.ToString()), ""))
                        {
