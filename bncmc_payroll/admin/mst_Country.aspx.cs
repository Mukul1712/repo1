...
{
    public partial class mst_Country : System.Web.UI.Page
    {
        private string form_PmryCol = "MiscID";
        private string form_tbl = "tbl_MiscellaneousMaster";
        private string Grid_fn = "fn_MiscCountryView()";
        static int iModuleID = 0;

        protected void Page_Load(object sender, EventArgs e)
        {
            CommonLogic.SetMySiteName(this, "Admin :: Country Master", true, true, true);
            if (!Page.IsPostBack)
            {
                iModuleID = Localization.ParseNativeInt(DataConn.GetfldValue("SELECT ModuleID FROM tbl_ModuleSettings Where PageLink='mst_Country.aspx'"));
            }
            ...
            txtCountry.Attributes.Add("onFocus", "nextfield ='" + btnSubmit.ClientID + "';");
            #endregion
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                strNotIn = " MiscID <>" + iPmryID + " and GroupID= 1";
            }
            catch
            {
                iPmryID = 0;
            ...
            else
            {
                ...
                if (iPmryID == 0)
                {
                    strQry = string.Format("insert into " + form_tbl + " values({0}, -1, 1)", CommonLogic.SQuote(txtCountry.Text.Trim().ToUpper()));
                }
                else
                {
                    strQry = string.Format("Update " + form_tbl + " set MiscName = {0} Where MiscID = '{1}'", CommonLogic.SQuote(txtCountry.Text.Trim().ToUpper()), iPmryID);
                }
                if (DataConn.ExecuteSQL(strQry.Replace("''", "Null"), iModuleID, 0) == 0)
                {
                    ...
                    if (iPmryID == 0)
                    {
                        AlertBox("Country Added successfully...", "", "");
                    }
                    else
                    {
                        AlertBox("Country Updated successfully...", "", "");
                    }
                    ClearContent();
                }
                else
                {
                    AlertBox("Error occurs while Adding/Updateing Country Name, please try after some time...", "", "");
                }
            }
        }
        ...
        protected void grdDtls_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "RowUpd":
                        txtCountry.Text = DataConn.GetfldValue(string.Concat(new object[] { "Select MiscName from ", form_tbl, " where ", form_PmryCol, " = ", e.CommandArgument, ";--" }));
                        ViewState["PmryID"] = e.CommandArgument;
                        break;

                    case "RowDel":
                        DataConn.ExecuteSQL(string.Format("Delete from tbl_MiscellaneousMaster where MiscID= " + e.CommandArgument + ";--", new object[0]), iModuleID, 0);
                        viewgrd(10);
                        ClearContent();
