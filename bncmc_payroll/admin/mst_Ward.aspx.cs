...
{
    public partial class mst_Ward : System.Web.UI.Page
    {
        private string form_PmryCol = "WardID";
        private string form_tbl = "tbl_WardMaster";
        private string Grid_fn = "tbl_WardMaster";
        static int iModuleID = 0;

        protected void Page_Load(object sender, EventArgs e)
        {
            CommonLogic.SetMySiteName(this, "Admin :: Ward Master", true, true, true);
            if (!Page.IsPostBack)
            {
                iModuleID = Localization.ParseNativeInt(DataConn.GetfldValue("SELECT ModuleID FROM tbl_ModuleSettings Where PageLink='mst_Ward.aspx'"));
            }
            ...
            txtWardName.Attributes.Add("onFocus", "nextfield ='" + txtAddress.ClientID + "';");
            txtAddress.Attributes.Add("onFocus", "nextfield ='" + txtPhone.ClientID + "';");
            txtPhone.Attributes.Add("onFocus", "nextfield ='" + btnSubmit.ClientID + "';");
            #endregion
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (!commoncls.IsUniqueEntry(form_tbl, "WardName", txtWardName.Text.Trim().ToUpper(), strNotIn))
            {
                AlertBox("Duplicate Ward Name Not Allowed !", "", "");
            }
            else
            {
                string strQry;
                if (iPmryID == 0)
                {
                    strQry = string.Format("Insert into {7} values({0}, {1}, {2}, {3}, {4}, {5}, {6})",
                                CommonLogic.SQuote(txtWardName.Text.Trim()).ToUpper(), CommonLogic.SQuote(txtWardNameInMarathi.Text.Trim()).ToUpper(), CommonLogic.SQuote(txtAddress.Text.Trim()), txtPhone.Text.Trim(),
                ...
                {
                    strQry = string.Format("Update {8} Set WardName = {0}, WardNameInMarathi={1}, Address = {2}, PhoneNo = {3}, IsActive={4}, UserID = {5}, UserDate = {6} Where WardID = {7}",
                             CommonLogic.SQuote(txtWardName.Text.Trim()).ToUpper(), CommonLogic.SQuote(txtWardNameInMarathi.Text.Trim()).ToUpper(), CommonLogic.SQuote(txtAddress.Text.Trim()), txtPhone.Text.Trim(),
                             (chkIsActive.Checked == true ? 1 : 0), LoginCheck.getAdminID(), CommonLogic.SQuote(Localization.ToSqlDateString(DateTime.Now.Date.ToString())), iPmryID, form_tbl);
                }

                double dblID = DataConn.ExecuteSQLMaster(strQry.Replace("''", "NULL"), iPmryID == 0, iModuleID, 0);
                if (dblID != -1)
                {
                    string SqlUpd = string.Format("Update {0} SET WardNameInMarathi=@var Where {1}={2};", form_tbl, form_PmryCol, (iPmryID == 0 ? dblID : iPmryID));
                    string var= txtWardNameInMarathi.Text.Trim();
                    ...
                    if (iPmryID == 0)
                    {
                        AlertBox("Ward Added successfully...", "", "");
                    }
                    else
                    {
                        AlertBox("Ward Updated successfully...", "", "");
                    }
                    ClearContent();
                }
                else
                {
                    AlertBox("Error occurs while Adding/Updateing Ward Name, please try after some time...", "", "");
                }
            }
        }
                ...
                switch (e.CommandName)
                {
                    case "RowUpd":
                        using (IDataReader iDr = DataConn.GetRS(string.Format("Select * from {0} where {1} = {2} ", form_tbl, form_PmryCol, e.CommandArgument)))
                        {
                            if (iDr.Read())
                            {
                                txtWardName.Text = iDr["WardName"].ToString();
                                txtWardNameInMarathi.Text = iDr["WardNameInMarathi"].ToString();
                    ...
                    case "RowDel":
                        if (commoncls.IsDeleted(commoncls.ComboType.Ward, Localization.ParseNativeInt(e.CommandArgument.ToString()), ""))
                        {
